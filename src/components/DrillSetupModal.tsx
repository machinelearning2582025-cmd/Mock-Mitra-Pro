      ...
      {isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-xl bento-card bg-[#0c0f17] border-brand/20 p-6 sm:p-10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
          >
            ...
            <header className="flex justify-between items-center mb-8 relative z-10 shrink-0">
              <div>
                ...
                <h2 className="text-xl sm:text-2xl font-black text-white">Daily Practice Setup</h2>
              </div>
              <button 
                onClick={onClose}
            ...
            <div className="space-y-6 relative z-10 overflow-y-auto pr-2 custom-scrollbar flex-grow">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {(['Easy', 'Medium', 'Hard'] as const).map((level) => (
                    <button
                      ...
                      className={`py-3 px-4 rounded-xl font-bold text-xs uppercase tracking-wider transition-all border-2 ${
                        difficulty === level 
              ...
              <div className="space-y-3">
                ...
                <textarea 
                  ...
                  className="w-full h-18 sm:h-24 px-5 py-4 bg-slate-900/50 border-2 border-white/5 rounded-2xl focus:border-brand/50 outline-none transition-all text-sm text-slate-300 placeholder:text-slate-700 resize-none"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <div 
                  ...
                  className="group cursor-pointer p-6 border-2 border-dashed border-white/5 bg-white/[0.01] hover:bg-white/[0.03] hover:border-brand/30 rounded-2xl transition-all flex flex-col items-center justify-center gap-3"
                >
                  <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center border border-white/10 group-hover:border-brand/30 transition-all">
                    <Upload className="w-5 h-5 text-slate-500 group-hover:text-brand transition-colors" />
                  </div>
                  <div className="text-center">
                    <p className="text-xs font-bold text-slate-400 group-hover:text-white transition-colors">Select or drop photos/PDFs</p>
                    <p className="text-[9px] text-slate-600 font-bold uppercase mt-1">Files (Max 5)</p>
                  </div>
                  <input 
                    type="file" 
                    multiple 
