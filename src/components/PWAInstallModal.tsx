...
export default function PWAInstallModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#0A0C10]/90 backdrop-blur-md">
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 15 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.95, y: 15 }}
        className="relative w-full max-w-md bento-card bg-[#0F1117] border border-brand/30 p-6 sm:p-8 shadow-2xl overflow-hidden flex flex-col rounded-3xl"
      >
        ...
        <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-5">
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              <h3 className="text-sm font-black uppercase tracking-wider text-white">Install App 📱</h3>
        ...
        <div className="space-y-4 text-xs font-semibold text-slate-300">
          <p className="text-slate-400 leading-normal font-sans text-center sm:text-left">
          ...
          <div className="space-y-3.5 mt-2">
            ...
            {deferredPrompt ? (
              <div className="p-3.5 bg-brand/5 border border-brand/20 rounded-xl flex items-start gap-3">
                <Info className="w-4 h-4 text-brand shrink-0 mt-0.5" />
                <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                  Aapke browser me <span className="text-white font-bold">direct quick installation</span> support available hai. Neeche diye gaye <span className="text-brand-light font-bold">"Install Now"</span> button par click karein.
                </p>
              </div>
            ) : isIOS ? (
              <div className="p-3.5 bg-white/[0.02] border border-white/5 rounded-xl">
                ...
                <p className="text-[11px] text-slate-400 leading-relaxed pl-3.5 font-sans font-medium">
                  Safari browser me bottom bar par <span className="text-white font-bold">Share Button (<Share2 className="inline-block w-3.5 h-3.5 mx-1" />)</span> par touch karein, phir neeche scroll karke <span className="text-brand-light font-bold">"Add to Home Screen"</span> touch karein.
                </p>
              </div>
            ) : (
              <div className="p-3.5 bg-white/[0.02] border border-white/5 rounded-xl">
                ...
                <p className="text-[11px] text-slate-400 leading-relaxed pl-3.5 font-sans font-medium">
                  Chrome/Edge/Firefox menu <span className="text-white font-bold">(⋮ teen dots)</span> me click karke <span className="text-brand-light font-bold">"Install App"</span> ya <span className="text-brand-light font-bold">"Add to Home screen"</span> select karein.
                </p>
              </div>
            )}
          </div>

          <div className="pt-4 border-t border-white/5 flex flex-col sm:flex-row gap-2 mt-4">
            <button
              onClick={onClose}
              className="w-full sm:w-1/2 py-3 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all cursor-pointer text-center"
            >
              Later
            </button>
