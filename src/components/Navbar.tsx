...
export default function Navbar({ 
  ...
  return (
    <nav className="sticky top-0 z-50 glass border-b border-slate-200 dark:border-white/5 px-4 sm:px-6 py-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="text-lg sm:text-2xl font-black font-display tracking-tight text-slate-900 dark:text-white uppercase leading-none">
              ...
              <>
                {!firebaseUser && onLoginWithGoogle && (
                  isWrapper ? (
                    <button
                      onClick={onAccountClick}
                      className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 bg-brand/10 hover:bg-brand/20 border border-brand/30 text-brand dark:text-brand-light font-black rounded-xl text-[9px] sm:text-[10px] uppercase tracking-wider transition-all scale-95 hover:scale-100 cursor-pointer shrink-0"
                    >
                    ...
                    <button
                      onClick={onAccountClick}
                      className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 bg-indigo-600/10 hover:bg-indigo-600/25 border border-indigo-500/30 text-indigo-700 dark:text-white font-black rounded-xl text-[9px] sm:text-[10px] uppercase tracking-wider transition-all scale-95 hover:scale-100 cursor-pointer shrink-0"
                    >
                      <svg className="w-3.5 h-3.5 text-indigo-500 dark:text-indigo-400" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div className="hidden sm:flex flex-col items-end">
                  ...
                  {!firebaseUser && (
                    <span className="text-[8px] font-black uppercase tracking-widest text-indigo-500 dark:text-indigo-400 mt-1">
                      Guest Mode
                    </span>
                  )}
                </div>
                ...
                <button 
                  onClick={onLogout} 
                  className="p-2 sm:px-3 sm:py-2 text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all"
                  title="Logout"
                >
