  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 py-4 sm:py-8">
      ...
      <header className="mb-5 sm:mb-6 flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-400 font-medium mt-0.5">
            ...
            {profile.customExamDetails && (
              <span className="text-[11px] text-slate-500">({profile.customExamDetails})</span>
            )}
          </div>
        </div>
        ...
        <button
          onClick={onMenuClick}
          className="w-10 h-10 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 flex items-center justify-center transition-all cursor-pointer shrink-0"
          title="Menu"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 sm:gap-4">
        ...
        <div className="md:col-span-12 lg:col-span-7 bento-card p-5 sm:p-6 flex flex-col justify-between">
          <div>
            ...
            <div className="text-4xl sm:text-5xl font-black text-white mt-1 mb-2">
        ...
        <div className="md:col-span-12 lg:col-span-5 bento-card p-5 sm:p-6 flex flex-col">
          <div className="flex items-center justify-between mb-3 pb-2 border-b border-white/5">
            <span className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
              <TrendingUp className="w-4 h-4 text-emerald-400" /> Topics
            </span>
          ...
          <div className="space-y-3 overflow-y-auto max-h-[160px] pr-1 custom-scrollbar">
            {topicAnalysis.length > 0 ? (
              topicAnalysis.map((topic, i) => (
                <div key={i} className="space-y-1">
                  <div className="flex justify-between text-xs font-medium text-slate-300">
                    ...
                    <span className={`font-mono text-[11px] ${topic.hasData ? 'text-white font-bold' : 'text-slate-600'}`}>
                      {topic.hasData ? `${topic.score}%` : '—'}
                    </span>
                  </div>
                  <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full ${topic.score >= 75 ? 'bg-emerald-500' : topic.score >= 50 ? 'bg-brand' : 'bg-slate-700'}`}
              ...
              <div className="text-center py-4 text-xs text-slate-600">No topics available</div>
            )}
          </div>
        </div>
          ...
          <div className="md:col-span-12 lg:col-span-5 bento-card p-5 sm:p-6 flex flex-col">
            <div className="flex items-center justify-between mb-3 pb-2 border-b border-white/5">
              <span className="text-xs font-bold text-white flex items-center gap-1.5">
                <AlertTriangle className="text-amber-400 w-4 h-4" /> Focus Areas
              </span>
              <button
            ...
            <div className="space-y-2 overflow-y-auto max-h-[220px] pr-1 custom-scrollbar">
              {gaps.map((gap, i) => (
                <div 
                  ...
                  className="p-2.5 bg-slate-900/60 rounded-xl border border-white/5 hover:border-amber-500/30 transition-all flex items-center justify-between cursor-pointer"
                >
                  <div className="min-w-0 pr-2">
                    <div className="text-xs font-medium text-slate-200 truncate">{gap.name}</div>
                    <div className="text-[10px] text-amber-400/90 font-semibold">{gap.status}</div>
                  </div>
            ...
            {history.length > 0 ? (
              history.slice().reverse().map((test, i) => (
                <div 
                  ...
                  className="p-3 bg-slate-900/60 rounded-xl border border-white/5 hover:border-brand/30 transition-all flex items-center justify-between cursor-pointer"
                >
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs text-slate-600">/{test.total}</span>
                  </div>
                </div>
              ))
            ) : (
              <div className="text-center py-8 text-xs text-slate-500">No test history yet</div>
            )}
          </div>
        </div>
